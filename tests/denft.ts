...
describe("denft", () => {
  ...
  // Test data constants
  const TEST_FILE_HASH = Array.from({ length: 32 }, (_, i) => i + 1);
  ...
  before(async () => {
    ...
    // File record PDA should use authority.publicKey, not uploader.publicKey
    [fileRecordPDA] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("file"),
        authority.publicKey.toBuffer(), // Fixed: use authority instead of uploader
  ...
  describe('record file access', () => {
    it("Should record read access successfully", async () => {
      ...
      const updatedFileRecord = await program.account.fileRecord.fetch(fileRecordPDA);
      assert.equal(updatedFileRecord.accessCount.toNumber(), 2);
    });
  ...
  describe('Edge Cases and Complex Scenarios', () => {
    it("Should handle multiple file uploads for same user", async () => {
      ...
      const updatedUserAccount = await program.account.userAccount.fetch(userAccountPDA);
      const expectedStorage = 2048 + 4096 + 8192 + 512;
      
      assert.equal(updatedUserAccount.fileCount, 4);
    ...
    it("Should handle access permission expiration correctly", async () => {
      ...
      try {
        ...
        assert.include(error.toString(), "Access revoked");
      }
    });
